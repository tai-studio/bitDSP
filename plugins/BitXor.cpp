...
BitXor::BitXor() {
    if (isAudioRateIn(0)) { // ax
        if (isAudioRateIn(1)) { // aa
            mCalcFunc = make_calc_function<BitXor, &BitXor::next_aa>();
            next_aa(1);
        } else { // ak
            mCalcFunc = make_calc_function<BitXor, &BitXor::next_ak>();
            next_ak(1);
        }
    } else { // kx
        if (isAudioRateIn(1)) { // ka
            mCalcFunc = make_calc_function<BitXor, &BitXor::next_ka>();
            next_ka(1);
        } else { // ak
            mCalcFunc = make_calc_function<BitXor, &BitXor::next_kk>();
            next_kk(1);
        }
    }
...
void BitXor::next_aa(int nSamples) {
...
void BitXor::next_ak(int nSamples) {
    const float* fp_input1 = in(0);
...
void BitXor::next_ka(int nSamples) {
...
void BitXor::next_kk(int nSamples) {
    const float f_in1 = in0(0);
    const float f_in2 = in0(1);
    float* fp_output = out(0);
